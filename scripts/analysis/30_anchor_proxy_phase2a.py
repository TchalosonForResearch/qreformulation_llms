#!/usr/bin/env python3
"""Build and evaluate the first corrected LegalBench anchor-preservation proxy."""
...
EPS = 1e-12
PRIMARY_SIGNAL = "combined_anchor_retention_v2"
METRIC_DELTAS = (
    "delta_Recall@10",
    "delta_Recall@100",
    "delta_MRR@10",
...
# Generic legal/document descriptors are not source identifiers.
DOCUMENT_GENERIC = {
    "consider", "agreement", "agreements", "contract", "contracts",
    "privacy", "policy", "policies", "non", "disclosure",
    "nondisclosure", "nda", "mutual", "acquisition", "merger",
    "branding", "endorsement", "licensing", "license", "hosting",
    "business", "promotion", "maintenance", "network", "build",
    "media", "cologuard", "parent", "target", "the", "and",
}
CORPORATE_SUFFIXES = {
...
def entity_phrases(original_text: object) -> List[str]:
    """
    Parse the highly regular LegalBench-RAG mini query preamble.

    Examples:
    - "Agreement between X and Y; ..."
    - 'Parent "X" and Target "Y"; ...'
    - 'Consider "Fiverr"\'s privacy policy; ...'
    - "Consider M5-Systems' Non-Disclosure Agreement; ..."
    """
    prefix = str(original_text).split(";", 1)[0]
    phrases: List[str] = []

    # Quoted party/application names.
    ...
    if " between " in lower:
        ...
        parts = re.split(r"\s+and\s+|,\s*", tail, flags=re.I)
...
def entity_anchor_tokens(original_text: object) -> List[str]:
    ...
    for phrase in entity_phrases(original_text):
        phrase_tokens = simple_tokens(phrase)
        ...
        # "CO" is occasionally a genuine short party label in ContractNLI.
        if phrase.strip().upper() == "CO":
            anchors.append("co")
...
def path_overlap_anchor_tokens(
    ...
    original_compact = re.sub(r"[^a-z0-9]+", "", ascii_lower(original_text))
    anchors = []

    for token in path_tokens(file_path):
        if token in original_set:
            anchors.append(token)
        elif len(token) >= 4 and token in original_compact:
            anchors.append(token)
    ...
    *,
    ...
    unique = frame[
        [
            ...
            "original_text",
            "file_paths",
            "path_anchor_tokens_v2",
            "entity_anchor_tokens_v2",
            "combined_anchor_tokens_v2",
        ]
...
def main() -> None:
    ...
    frame["path_anchor_tokens_v2"] = path_anchor_column
    frame["entity_anchor_tokens_v2"] = entity_anchor_column
    frame["combined_anchor_tokens_v2"] = combined_anchor_column
    frame["path_anchor_count_v2"] = [
        0 if not text else len(text.split(", ")) for text in path_anchor_column
    ]
    frame["entity_anchor_count_v2"] = [
        0 if not text else len(text.split(", ")) for text in entity_anchor_column
    ]
    frame["combined_anchor_count_v2"] = [
        0 if not text else len(text.split(", ")) for text in combined_anchor_column
    ]

    frame["kept_path_anchor_tokens_v2"] = kept_path_column
    frame["dropped_path_anchor_tokens_v2"] = dropped_path_column
    frame["path_anchor_retention_v2"] = path_retention_column

    frame["kept_entity_anchor_tokens_v2"] = kept_entity_column
    frame["dropped_entity_anchor_tokens_v2"] = dropped_entity_column
    frame["entity_anchor_retention_v2"] = entity_retention_column

    frame["kept_combined_anchor_tokens_v2"] = kept_combined_column
    frame["dropped_combined_anchor_tokens_v2"] = dropped_combined_column
    frame["combined_anchor_retention_v2"] = combined_retention_column
    ...
    summary = (
        ...
        .agg(
            ...
            avg_path_anchor_count_v2=("path_anchor_count_v2", "mean"),
            avg_entity_anchor_count_v2=("entity_anchor_count_v2", "mean"),
            avg_combined_anchor_count_v2=("combined_anchor_count_v2", "mean"),
            avg_path_anchor_retention_v2=("path_anchor_retention_v2", "mean"),
            avg_entity_anchor_retention_v2=("entity_anchor_retention_v2", "mean"),
            avg_combined_anchor_retention_v2=(
                "combined_anchor_retention_v2", "mean"
            ),
            zero_combined_retention_rate=(
                "combined_anchor_retention_v2",
        ...
        subgroup_rows.append(
            {
                ...
                "p_raw_approx": approximate_spearman_pvalue(rho, len(group)),
    ...
    subgroup["p_holm_24"] = holm_adjust(subgroup["p_raw_approx"].tolist())
    ...
    signals = (
        "combined_anchor_retention_v2",
        "path_anchor_retention_v2",
        "entity_anchor_retention_v2",
    )
    ...
    fig_pdf = figs_dir / "figure4_anchor_preservation_recall100.pdf"
    fig_png = figs_dir / "figure4_anchor_preservation_recall100.png"
...
"""
    ...
    print("[6/6] Saving Phase 2A artifacts...")
    per_query_out = output_dir / "anchor_preservation_per_query_v2.csv"
    summary_out = output_dir / "anchor_proxy_summary_v2.csv"
    subgroup_out = output_dir / "anchor_proxy_subgroup_correlations.csv"
    pooled_out = output_dir / "anchor_proxy_pooled_clustered_correlations.csv"
    audit_out = output_dir / "anchor_proxy_structural_audit.txt"
    manifest_out = output_dir / "anchor_proxy_input_manifest.csv"
    ...
    summary.to_csv(tables_dir / "anchor_proxy_summary_v2.csv", index=False)
    subgroup.to_csv(
        tables_dir / "anchor_proxy_subgroup_correlations.csv",
        index=False,
    )
    pooled.to_csv(
        tables_dir / "anchor_proxy_pooled_clustered_correlations.csv",
        index=False,
    )
    validation.to_csv(
        tables_dir / "anchor_proxy_validation_sample.csv",
        index=False,
    )
    (
        tables_dir / "anchor_proxy_validation_instructions.md"
    ...
    new_zero_rows = int(frame["combined_anchor_count_v2"].eq(0).sum())

    with audit_out.open("w", encoding="utf-8") as handle:
        handle.write("PHASE 2A — ANCHOR PROXY AUDIT\n")
        handle.write("================================\n\n")
        ...
        handle.write(
            "The old anchor denominator may contain filepath tokens that "
        ...
        handle.write(
            f"Rows with v2 combined_anchor_count = 0: {new_zero_rows}\n\n"
        )
        ...
        handle.write(
            "This is a lexical proxy. The manual validation worksheet "
    ...
    print(f"V2 zero-anchor rows:           {new_zero_rows:,}")
    print(
        "Primary pooled rho:           "
