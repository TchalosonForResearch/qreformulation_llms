"""Run the final dense and hybrid LegalBench baselines and preserving fusions."""
...
def load_run(method: str) -> pd.DataFrame:
    """Load a previously generated run and validate its schema."""
    path = RUNS_DIR / f"{method}.tsv"
    if not path.exists():
        raise FileNotFoundError(
            f"Missing existing run required by --reuse-existing-runs: {path}"
        )
    ...
    if missing:
        raise ValueError(f"Invalid run file {path}; missing columns: {sorted(missing)}")
    ...
    run_df["score"] = pd.to_numeric(run_df["score"], errors="raise").astype(float)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Reuse dense and hybrid TSV runs already present in "
            "runs/legalbench/dense_hybrid. This skips model loading, "
            "embedding, BM25 indexing and retrieval, and only recomputes "
            "dense/hybrid RRF fusion plus evaluation/statistics."
        ),
    )
...
def main() -> None:
    ...
    print(f"Reuse runs:     {args.reuse_existing_runs}")
    ...
    # BM25 is needed only when producing hybrid runs from scratch.
    bm25: BM25Okapi | None = None
    if not args.reuse_existing_runs:
        print("Building BM25 index for hybrid scoring...")
        bm25 = BM25Okapi(
            [simple_tokenize(t) for t in tqdm(doc_texts, desc="Tokenizing corpus")]
        )
    else:
        print(
            "Reuse mode enabled: skipping model loading, document/query encoding, "
    ...
    for model_key in model_keys:
        ...
        print("=" * 90)
        # Retrieve from scratch, or reuse the TSV runs already produced.
        ...
        if args.reuse_existing_runs:
            for view_name in query_views:
                ...
                hybrid_method = (
                    f"hybrid_{spec.short_name}_{args.index_mode}_"
                    f"dw{dense_weight_tag}_{view_name}"
                )
                dense_runs[view_name] = load_run(dense_method)
                hybrid_runs[view_name] = load_run(hybrid_method)
        else:
            if bm25 is None:
                raise RuntimeError("BM25 index was not initialised.")
            ...
            cache_base = safe_cache_name(
                f"{spec.short_name}_{args.index_mode}_{len(doc_ids)}docs"
            )
            ...
            for view_name, qdict in query_views.items():
                ...
                q_cache = CACHE_DIR / (
                    "query_"
                    + safe_cache_name(
                        spec.short_name + "_" + args.index_mode + "_" + view_name
                    )
                    + ".npy"
                )
            ...
            # Release model memory after all query views have been encoded.
            del model
        ...
        # Dense original-preserving RRF fusion.
        fusion_specs = {
            "original_deepseek_keyword_gpt_legal": [
                "original",
                "deepseek_keyword",
                "gpt_legal",
            ],
        ...
        for fusion_name, views in fusion_specs.items():
            method = (
                f"dense_rrf_{spec.short_name}_{args.index_mode}_{fusion_name}"
            )
            fused = rrf_fuse(
                method,
                [dense_runs[v] for v in views],
                rrf_k=args.rrf_k,
                top_k=args.top_k,
            )
